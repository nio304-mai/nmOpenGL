...
solution "demo3d-host"
   ...
   project "demo3d-host"
        ...
        configuration {"Debug","x86"}
			...
			targetsuffix ("-x86d")
			architecture "x32"
			defines { "DEBUG", "TEXTURE_ENABLED",  "EMULATION"}
			symbols  "On" 
		configuration {"Release","x86"}
			links { "vshell.lib", "nmpp-x86.lib","hal-virtual-x86.lib"}
			targetsuffix ("-x86")
			architecture "x32"
			defines { "NDEBUG", "TEXTURE_ENABLED",  "EMULATION"}
			symbols  "Off" 
...
solution "demo3d-target0"
   ...
   project "demo3d-target0"
      kind "ConsoleApp"
	  language "C++"
      files { "../../include/*.h","../src_target0/*.cpp", "../../src_proc0/pc/*.*","../../src_proc0/common/*.*","../../src_proc0/nmgl/*.*","../../nmglvs_mc12101-gcc/src_nmc0/*.*" }
...
solution "demo3d-target1"
   ...
   project "demo3d-target1"
      kind "ConsoleApp"
	  language "C++"
      files { "../../include/*.h","../src_target1/*.cpp", "../../src_proc1/pc/*.*","../../src_proc1/common/*.*", "../../nmglvs_mc12101-gcc/src_nmc1/*.*"  }
	  libdirs { "$(NMPP)/lib","$(HAL)/lib"}
	  includedirs { "$(MC12101)/include","$(HAL)/include","$(NMPP)/include","../../include"}
	  configuration {"Debug","x86"}
			links { "nmpp-x86d.lib","hal-virtual-x86d.lib"}
